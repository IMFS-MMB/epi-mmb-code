/* 
New Keynesian model with SIR-type epidemic dynamics
The Limited Power of Monetary Policy in a Pandemic, Antoine Lepetit and Cristina Fuentes-Albero, November 2021
*/

var t s i r d R0 tau w infl ffr ffr_shadow y lambda c_s c_i c_r n_s n_i n_r V_s V_i V_r V_agg pi_s1 pi_s2 pi_s3 counter;
@#include "commonVar.mod"    // added by epi-mmb team

varexo mu mp_shock pi_s1_shock pi_s2_shock pi_s3_shock ffr_trigger counter_shock; 

parameters pi_s1_ss pi_s2_ss pi_s3_ss pi_r pi_d R0_ss share
           beta teta phip delta_infl delta_gap xi varphi sigma phi A infl_target flow_value_life
...
pi_s3_ss = R0_ss*(pi_r+pi_d)/(1 + 2*(share/(1-share))*(1+share/(1-share))/(1 - (share/(1-share))^2));
pi_s1_ss = (share/(1-share))*(1+share/(1-share))*pi_s3_ss/((1 - (share/(1-share))^2)*((A*n_s_init)^2));
pi_s2_ss = (share/(1-share))*(1+share/(1-share))*pi_s3_ss/((1 - (share/(1-share))^2)*((n_s_init)^2));
...
V_s_init = ((c_s_init^(1-sigma))/(1-sigma) - xi*(n_s_init^(1+1/varphi))/(1+1/varphi) + flow_value_life + lambda_init*(w_init*n_r_init - c_r_init))/(1-beta);
...
V_s_end = ((c_s_end^(1-sigma))/(1-sigma) - xi*(n_s_end^(1+1/varphi))/(1+1/varphi) + flow_value_life + lambda_end*(w_end*n_r_end - c_r_end))/(1-beta);
...
[name = 'pi_s1']
pi_s1 = pi_s1_ss + pi_s1_shock;

[name = 'pi_s2']
pi_s2 = pi_s2_ss + pi_s2_shock;

[name = 'pi_s3']
pi_s3 = pi_s3_ss + pi_s3_shock;
...
1 - teta + teta*w/A - phip*infl*(infl-infl_target) + beta*phip*infl(+1)*(infl(+1)-infl_target)*y(+1)/y = 0;
...
1+ffr = (counter < 1)*(1+ffr_shadow) + (counter >=1)*(counter <= 32)*(1+ffr_trigger) + (counter > 32)*(counter < 58)*1 + (counter >= 58)*(1+ffr_shadow);

[name = 'Time index']
counter = counter(-1) + counter_shock;

// Households

[name = 'Marginal utility of consumption - susceptibles']                      
lambda*(1+mu) = c_s^(-sigma) +  pi_s1*i(-1)*c_i*beta*(V_i(+1)-V_s(+1));
...
lambda*w = xi*(n_s^(1/varphi)) - pi_s2*i(-1)*n_i*beta*(V_i(+1)-V_s(+1));
...
V_s = (c_s^(1-sigma))/(1-sigma) - xi*(n_s^(1+1/varphi))/(1+1/varphi) + flow_value_life + lambda*(w*n_s - (1+mu)*c_s) + (1-tau)*beta*V_s(+1) + tau*beta*V_i(+1);
...
[name = 'Aggregate intertemporal utility']
V_agg = s(-1)*((c_s^(1-sigma))/(1-sigma) - xi*(n_s^(1+1/varphi))/(1+1/varphi) + flow_value_life)
       + i(-1)*((c_i^(1-sigma))/(1-sigma) - xi*(n_i^(1+1/varphi))/(1+1/varphi) + flow_value_life)
       + r(-1)*((c_r^(1-sigma))/(1-sigma) - xi*(n_r^(1+1/varphi))/(1+1/varphi) + flow_value_life)
       + beta*V_agg(+1);
...
V_i = V_i_end; 
V_r = V_r_end;
V_agg = (s_end*((c_s_end^(1-sigma))/(1-sigma) - xi*(n_s_end^(1+1/varphi))/(1+1/varphi) + flow_value_life)
       + r_end*((c_r_end^(1-sigma))/(1-sigma) - xi*(n_r_end^(1+1/varphi))/(1+1/varphi) + flow_value_life))/(1-beta);
pi_s1 = 0;
pi_s2 = 0;
pi_s3 = 0;
pi_s1_shock = -pi_s1_ss;
pi_s2_shock = -pi_s2_ss;
pi_s3_shock = -pi_s3_ss;
...
values 0;
var pi_s2_shock;
periods 1:4,5,6,7,8:500;
values 0,-0.02,-0.04,-0.06,-0.08;
var pi_s3_shock;
periods 1:2,3:4,5:9,10:14,15:24,25:500;
values -0.19,-0.195,-0.2025,-0.205,-0.2075,-0.21;
var counter_shock;
periods 1:500;
values 1;
end;
