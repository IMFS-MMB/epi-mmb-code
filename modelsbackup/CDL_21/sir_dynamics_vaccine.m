...
% in model with vaccine and medical treatment
function [pol_mkt_s_opt,wage_s_opt,RnotSIR,I,Ih,Im,S,Sh,Sm,R,D,V_r,V_i,V_sh,V_sm,Cs,Ci,Cr,C,G,Y,theta_total] = sir_dynamics_vaccine(pol_mkt_s,...
    wage_s,sec_weight,sec_elas,occ_weight,occ_elas,tau_m,wage_sm,wage_sh,wage_r,wage_ih,wage_im,wage_i,pol_mkt_r,pol_mkt_i,...
...
delta_v = pi_par(7); % Weekly probability of vaccines
delta_t = pi_par(8); % Weekly probability of treatment
...
for t = 1:1:periods
    ...
    G(t,1) = sum( (tau_m(t,:).*wage_s(t,:).*Si(t,:).*(pol_mkt_s(t,:)==2) + tau_m(t,:).*wage_i(t,:).*Ii(t,:).*(pol_mkt_i(t,:)==2) ...
        + tau_m(t,:).*wage_r(t,:).*Ri(t,:).*(pol_mkt_r(t,:)==2)),2)/P(t,1);
        
    Cs(t,:) = (1-tau_m(t,:)).*wage_s(t,:).*Si(t,:).*(pol_mkt_s(t,:)==2) + wage_s(t,:).*Si(t,:).*(pol_mkt_s(t,:)==1) + G(t,1).*Si(t,:);
                        
    Ci(t,:) = (1-tau_m(t,:)).*wage_i(t,:).*Ii(t,:).*(pol_mkt_i(t,:)==2) + wage_i(t,:).*Ii(t,:).*(pol_mkt_i(t,:)==1) + G(t,1).*Ii(t,:);
    
    Cr(t,:) = (1-tau_m(t,:)).*wage_r(t,:).*Ri(t,:).*(pol_mkt_r(t,:)==2) + wage_r(t,:).*Ri(t,:).*(pol_mkt_r(t,:)==1) + G(t,1).*Ri(t,:);
...
% Last period
G(end,1) = sum((tau_m(end,:).*wage_s(end,:).*Si(end,:).*(pol_mkt_s(end,:)==2) + tau_m(end,:).*wage_i(end,:).*Ii(end,:).*(pol_mkt_i(end,:)==2) ...
        + tau_m(end,:).*wage_r(end,:).*Ri(end,:).*(pol_mkt_r(end,:)==2)) ,2)/P(end,1);
        
Cs(end,:) = (1-tau_m(end,:)).*wage_s(end,:).*Si(end,:).*(pol_mkt_s(end,:)==2) + wage_s(end,:).*Si(end,:).*(pol_mkt_s(end,:)==1) + G(end,1).*Si(end,:);
                        
Ci(end,:) = (1-tau_m(end,:)).*wage_i(end,:).*Ii(end,:).*(pol_mkt_i(end,:)==2) + wage_i(end,:).*Ii(end,:).*(pol_mkt_i(end,:)==1) + G(end,1).*Ii(end,:);
    
Cr(end,:) = (1-tau_m(end,:)).*wage_r(end,:).*Ri(end,:).*(pol_mkt_r(end,:)==2) + wage_r(end,:).*Ri(end,:).*(pol_mkt_r(end,:)==1) + G(end,1).*Ri(end,:);
...
V_r(end,:) = max([log( wage_sh(end,:) + G(end,1) ) ;log( (1-tau_m(end,:)).*wage_sm(end,:) + G(end,1) )],[],1)/(1-betta); % last period value of recovered
V_i(end,:) = max([log( wage_ih(end,:) + G(end,1) ) ;log( (1-tau_m(end,:)).*wage_im(end,:) + G(end,1) )],[],1)/(1-betta*(1-delta_t)*(1-pir-pid)) + V_r(end,:)/(1-betta*pir-betta*delta_t*(1-pir) );% last period value of infected

for t = periods:-1:1
   V_r(t,:) = max([log( wage_sh(t,:) + G(t,1) );log( (1-tau_m(t,:)).*wage_sm(t,:)  + G(t,1) )],[],1) + betta*V_r(t+1,:); % max between wage of market vs. home + next period value
   V_i(t,:) = max([log( wage_ih(t,:) + G(t,1) );log( (1-tau_m(t,:)).*wage_im(t,:)  + G(t,1) )],[],1) ...
       + betta*(pir+delta_t*(1-pir))*V_r(t+1,:) + betta*(1-delta_t)*(1-pir-pid)*V_i(t+1,:); % max between wage of market vs. home + next period value
end
...
%last period
V_sh(end,:) = log( wage_sh(end,:) + G(end,1) )/(1-betta);
V_sm(end,:) = log( (1-tau_m(end,:)).*wage_sm(end,:) + G(end,1) )/(1-betta);
...
for t = periods:-1:1
    for i = 1:n_occ
        V_sh(t,i) = log( wage_sh(t,i) + G(t,1) ) + betta*(1-delta_v)*(1-pi_t(t,1))*max([V_sh(t+1,i),V_sm(t+1,i)]) ...
            + betta*(1-delta_v)*pi_t(t,1)*V_i(t+1,i) + betta*delta_v*V_r(t+1,i);
        V_sm(t,i) = log( (1-tau_m(t,i)).*wage_sm(t,i) + G(t,1) ) + betta*(1-delta_v)*(pim(t,i)+pi_t(t,1))*V_i(t+1,i) ...
            + betta*(1-delta_v)*(1-pim(t,i)-pi_t(t,1))*max([V_sh(t+1,i),V_sm(t+1,i)]) + betta*delta_v*V_r(t+1,i);
