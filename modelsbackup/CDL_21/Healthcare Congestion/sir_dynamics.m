...
% in model with congestion in health care
...
kappa = pi_par(7); % Congestion parameter
...
for t = periods:-1:1
   ...
   V_i(t,:) = max([log( wage_ih(t,:) + G(t,1) );log( (1-tau_m(t,:)).*wage_im(t,:)  + G(t,1) )],[],1) + betta*pir*V_r(t+1,:) + betta*(1-pir-pid_t(t,1))*V_i(t+1,:); % max between wage of market vs. home + next period value
end
