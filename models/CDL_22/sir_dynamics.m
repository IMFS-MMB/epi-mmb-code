...
% in baseline model
function [pol_mkt_s_opt,wage_s_opt,RnotSIR,I,Ih,Im,S,Sh,Sm,R,D,V_r,V_i,V_sh,V_sm,Cs,Ci,Cr,C,G,Y,theta_total] = sir_dynamics(pol_mkt_s,...
...
mkt_risk = varepsilon.*(1+risk_prop); %transmission of infection risk from 
% market activity
%pre-allocate
...
V_i(end,:) = max([log( wage_ih(end,:) + G(end,1) ) ;log( (1-tau_m(end,:)).*wage_im(end,:) + G(end,1) )],[],1)/(1-betta*(1-pir-pid)) + V_r(end,:)/(1-betta*pir);% last period value of infected
...
for t = periods:-1:1
   ...
   V_i(t,:) = max([log( wage_ih(t,:) + G(t,1) );log( (1-tau_m(t,:)).*wage_im(t,:)  + G(t,1) )],[],1) + betta*pir*V_r(t+1,:) + betta*(1-pir-pid)*V_i(t+1,:); % max between wage of market vs. home + next period value
end
...
for t = periods:-1:1
    for i = 1:n_occ
        V_sh(t,i) = log( wage_sh(t,i) + G(t,1) ) + betta*(1-pi_t(t,1))*max([V_sh(t+1,i),V_sm(t+1,i)]) ...
            + betta*pi_t(t,1)*V_i(t+1,i);
        V_sm(t,i) = log( (1-tau_m(t,i)).*wage_sm(t,i) + G(t,1) ) + betta*(pim(t,i)+pi_t(t,1))*V_i(t+1,i) ...
...
RnotSIR=T(1)/I(1)/(pir+pid); % R_0 in endogenous model
end