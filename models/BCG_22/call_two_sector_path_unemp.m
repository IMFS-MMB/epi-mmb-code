...
for case_switch = 1
    ...
    share_working_while_lockdown = ones(Jgroups,nperiods);
    share_working_while_lockdown(2,:) = 0.15;
    share_working_while_lockdown(3,:) = 0.4;
   
    %share_working_while_lockdown(3,16:75) = 0.4-linspace(0,1,60)*0.075/(l2_ss/(l1_ss+l2_ss));
    %share_working_while_lockdown(3,76:165) = 0.4-0.075+linspace(0,1,90)*0.035/(l2_ss/(l1_ss+l2_ss));
    res_.work(2,1:months_lockdown*30) =  share1_lockdown;
    res_.work(3,1:months_lockdown*30) =  share2_lockdown;
    
    share1_add_lockdown = 0.04;
    share2_add_lockdown = 0.30;
...
%         = SIRmodel_Jgroups_func(betamat,nperiods,nperiods,irate,Ninit,gammap,thetap,deltap,omegap);
    ...
    legend('Reproduction rate, observed','Reproduction, model implied')
...
%%
    ...
    plot(res_.work(2,:))
    hold on
    plot(share_working_while_lockdown(2,:),'r--')
    plot(max(res_.work(2,:)-share_working_while_lockdown(2,:),0),'k')
    ...
    plot(share_working_while_lockdown(3,:),'r--')
    
    plot(max(res_.work(3,:)-share_working_while_lockdown(3,:),0),'k')
    ...
    % for 0.65
    %l2_path_lockdown(1:length(ur_table.UNRATEDIFF(5:end))) = -35/42*ur_table.UNRATEDIFF(5:end)*(l1_ss+l2_ss)/100;
    %l1_path_lockdown(1:length(ur_table.UNRATEDIFF(5:end))) = -7/42*ur_table.UNRATEDIFF(5:end)*(l1_ss+l2_ss)/100;
    
    % for 0.6
    
    months_unemp = length(ur_table.UNRATEDIFF(5:end));
    l2_path_lockdown(1:months_unemp) = -34/42*ur_table.UNRATEDIFF(5:end)*(l1_ss+l2_ss)/100;
    l1_path_lockdown(1:months_unemp) = -8/42*ur_table.UNRATEDIFF(5:end)*(l1_ss+l2_ss)/100;
    
    l2_path_lockdown(months_unemp+1:end) =  l2_path_lockdown(months_unemp)*rhop_l.^(1:(months-months_unemp));
    l1_path_lockdown(months_unemp+1:end) =  l1_path_lockdown(months_unemp)*rhop_l.^(1:(months-months_unemp));
...
%%    
    ...
    if slide_switch
        ...
        %dead_cum = day2month(sum(Dmat)');
        %dead_daily = [dead_cum(1); dead_cum(2:end)-dead_cum(1:end-1)];
        %dead_month = day2month(dead_daily)*30*100000/4;
        
        %day2month(sum(Cmat)'
        line1 = 100*[ ...
            s1_y_irf/s1_y_ss, -(s1_l1_irf+s1_l2_irf)/(s1_l1_ss+s1_l2_ss),...
            s1_c_irf/s1_c_ss, s1_in_irf/s1_in_ss,...
            s1_v1_irf/s1_v1_ss, s1_v2_irf/s1_v2_ss];
        
        line2 = 100*[...
            s2_y_irf/s2_y_ss, -s2_l2_irf/s2_l2_ss,...
            s2_c_irf/s2_c_ss, s2_in_irf/s2_in_ss,...
            nan*s1_v1_irf/s1_v1_ss, nan*s1_v2_irf/s1_v2_ss];
...
gdpf = [19253.959
19253.959*(1.016^(1/4))
19253.959*(1.016^(1/4))*(1.019^(1/4))
19253.959*(1.016^(1/4))*(1.019^(1/4))*(1.018^(1/4))];



gdp_gap = log(gdpf)-log(gdp);

% bottom line: GDP in Q2 should drop about 12 percent --- hit this mark to
% calibrate the minimum-scale parameter 
