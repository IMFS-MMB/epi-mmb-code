...
vars_paper      = {'util_a'; 'GDP_fix'; 'Cb'; 'N_a'; 'N_n'; 'R'; 'Fb'; 'sprqb'; 'f'; 'm'};
var_names_paper = {'Shock'; 'Real GDP'; 'Cons. Borrower'; 'Labor Sector A'; 'Labor Sector N'; 'Policy Rate'; 'Default Rate'; 'Credit Spread'; 'Mass of Firms'; 'Entrants'};
...
if options.shock_util_a == 1
    ...
    for ii = 1:length(vars)
        ...
        if strcmp(currvar,'T') || strcmp(currvar,'rot') || strcmp(currvar,'m')
            ...
            axis tight
            ylabel('level')
        elseif strcmp(currvar,'R') || strcmp(currvar,'sprqb') || strcmp(currvar,'Fb') 
...
if options.crisis_G == 1
    ...
    % Figure for paper
    vars_paper      = {'Bg'; 'GDP_fix'; 'Cb'; 'N_a'; 'N_n'; 'R'; 'Fb'; 'sprqb'; 'f'; 'm'};
    var_names_paper = {'Public Debt'; 'Real GDP'; 'Cons. Borrower'; 'Labor Sector A'; 'Labor Sector N'; 'Policy Rate'; 'Default Rate'; 'Credit Spread'; 'Mass of Firms'; 'Entrants'};
    ...
    for ii = 1:length(vars_paper)
        ...
        if strcmp(currvar,'T') || strcmp(currvar,'rot') || strcmp(currvar,'m')
            crisis = irf_store.nopol(currind,1:Tirf)';
            crisis_pol = irf_store.G(currind,1:Tirf)';
            plot(dateplot, [crisis, crisis_pol], 'Linewidth', 2), hold on
            plot(dateplot, detss.(currvar)*ones(size(time)), 'k--')
            title(currname)
            axis tight
            grid minor
        ...
        elseif strcmp(currvar,'Bg')
            crisis = exp(irf_store.nopol(currind,1:Tirf))';
            crisis_pol = exp(irf_store.G(currind,1:Tirf))';
            crisis_pol = 100*(crisis_pol-crisis)./detss.(currvar);
...
if options.crisis_varsigma == 1
    ...
    oo_.exo_simul(1:6,ind_ua) = data_shocks(1:6,ind_ua);
    oo_.exo_simul(2,ind_varsigma) = 0.1*policy_size*detss.GDP/detss.varsigma/(1-detss.N_n-detss.N_a);
    perfect_foresight_solver;
    oo_.exo_simul(:,:) = 0;
...
if options.bar_plots==1
    ...
    y = d_income;
    bar(X, y), title('Policy Impact on Total Income'), ylabel('% Change on Impact')
    ...
    for jj = 1:length(pols)
        ...
        subplot(3,2,jj)
        bar(X, y), title(pol_names{jj}), ylabel('% Change')
    end
