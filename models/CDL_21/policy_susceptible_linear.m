...
% model with risk-neutral preferences
function [pol_mkt_s_opt,wage_s_opt,RnotSIR,I,Ih,Im,S,Sh,Sm,R,D,V_r,V_i,V_sh,V_sm,Cs,Ci,Cr,C,G,Y,theta_total,error] ...
    = policy_susceptible_linear(pol_mkt_s_guess,wage_s_guess,sec_weight,sec_elas,occ_weight,occ_elas,tau_m,wage_sm,wage_sh,wage_r,...
...
while stop==0
    ...
    [pol_mkt_s_opt,wage_s_opt,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~] = sir_dynamics_linear(pol_mkt_s_guess,wage_s_guess,...
...
[pol_mkt_s_opt,wage_s_opt,RnotSIR,I,Ih,Im,S,Sh,Sm,R,D,V_r,V_i,V_sh,V_sm,Cs,Ci,Cr,C,G,Y,theta_total] ...
    = sir_dynamics_linear(pol_mkt_s_opt,wage_s_opt,sec_weight,sec_elas,occ_weight,occ_elas,tau_m,wage_sm,wage_sh,wage_r,...
    wage_ih,wage_im,wage_i,pol_mkt_r,pol_mkt_i,risk_prop,ini_pop,betta,pi_par,periods);
