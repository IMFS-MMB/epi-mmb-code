...
model; 
    ...
    // (5) Borrower SDF
    exp(mb) = pbetab*(exp(Cb(-1))/exp(Cb))^peis;

    // (6) Borrower Euler
    exp(Qb)*(1-max(0,lbd)^2) = (exp(mb(+1)))*(1-exp(Fb(+1)));

    // (7) Borrower budget constraint
    exp(Cb) + (exp(Bb(-1))/pchi)*(1-exp(Fb)) = exp(N_n)*exp(w_n)*(1-exp(tau_l)) + exp(N_a)*exp(w_a)*(1-exp(tau_l)) + (1 - exp(N_a) - exp(N_n))*exp(varsigma) + exp(Qb)*(exp(Bb)/pchi) + transfer - T;
    ...
    // (15) Saver SDF
    exp(ms) = pbeta*(exp(Cs(-1))/exp(Cs))^peis;
    ...
    exp(C) + exp(G) + pshare_a*exp(Psik)*exp(f(-1)) = exp(Y)*(1-rot);
    ...
    exp(C) = (1-pchi)*exp(Cs) + pchi*exp(Cb);

    // (23) Government BC
    exp(G) + exp(Bg(-1))/exp(Pi) + (1-exp(N_a)-exp(N_n))*exp(varsigma) + govt_wage*exp(N_a)*exp(w_a) + transfer + bailout = exp(N_n)*exp(w_n)*exp(tau_l) +  exp(N_a)*exp(w_a)*exp(tau_l) + ptau_k*(exp(Y)*(1-rot) - exp(w_n)*exp(N_n) + exp(N_a)*(exp(p_a)*exp(util_a) - exp(w_a)) - pshare_a*exp(Psik)*exp(f(-1))) + exp(Q)*exp(Bg) + T;
    ...
    // (31) Value of firm
    exp(ck) = exp(p_a)*exp(util_a)*exp(a) - exp(w_a) + govt_wage*exp(w_a) + exp(ms(+1))*(exp(Gk(+1))*exp(ck(+1)) - exp(Psik(+1)));
    ...
    (1-pchi)*exp(Cs_a) = exp(a)*exp(util_a)*pshare_a*exp(f);

    // (35) Saver consumption, a-sector
    exp(Cs_a) = (ppref_a*exp(Cs)^peis / exp(p_a))^(1/peis_a);
    ...
    exp(GDP) = exp(a)*(exp(N_n) + exp(p_a)*exp(util_a)*exp(N_a));
