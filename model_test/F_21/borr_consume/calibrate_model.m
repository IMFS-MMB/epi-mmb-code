...
detss.C_n = detss.Y - detss.G - par.share_a*detss.Psik*detss.f; 
...
% Consumption and wages, x1=Cb, x2=alpha
detss.G = par.goy*detss.Y;
borr_bc = detss.N_n*detss.w_n*(1-par.tau_l) + detss.N_a*detss.w_a*(1-par.tau_l) + (1-detss.N_n-detss.N_a)*par.varsigma + detss.Qb*detss.Bb/par.chi - (detss.Bb/detss.Pi/par.chi)*(par.gamma*(1-detss.Fb) + (1-par.gamma)*(1-par.Fb)*detss.Qb);

fCsn = @(x) (detss.C_n - par.chi*x(1))/(1-par.chi);
fCba = @(x) (x(2)*(x(1)^par.eis)/detss.p_a)^(1/par.eis_a);
fCsa = @(x) (x(2)*(fCsn(x)^par.eis)/detss.p_a)^(1/par.eis_a);

f1  = @(x) x(1) + detss.p_a*fCba(x) - borr_bc;
f2  = @(x) par.chi*fCba(x) + (1-par.chi)*fCsa(x) - detss.N_a;
F   = @(x) [f1(x); f2(x)];
x0  = [1/3; 1];
out = fsolve(F, x0);
...
detss.Cb = detss.Cb_n + detss.p_a*detss.Cb_a;
detss.Cs = detss.Cs_n + detss.p_a*detss.Cs_a;
detss.C_a = (par.chi*detss.Cb_a + (1-par.chi)*detss.Cs_a);
detss.C  = detss.C_n + detss.p_a*detss.C_a;
...
data_govt_wage = [0.0; 0.012556429; 0.014181524; 0.0];
...
shock.bailout   = 0;
shock.util_a    = 0.05*(1.0^(1/par.eis_a)); % initial guess for pandemic shock
